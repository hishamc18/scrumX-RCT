  ...
  return (
    <>
      ...
      <div>
        ...
        <Field as="textarea" name="description" placeholder="Describe your project..." className="resize-none border-2 text-sm border-gray-300 bg-offWhite mt-2 px-3 py-2 rounded-xl w-full text-primaryDark placeholder-placeholder focus:outline-none min-h-[80px]" />
      ...
      <div className="mb-2">
        ...
        <div className="flex mt-1 justify-between space-x-2 overflow-x-auto scrollbar-none">
          {imageOptions.map((image, index) => (
            <div key={index}  onClick={() => { setSelectedImage(image); setFieldValue("image", image); }}>
              <Image src={image} alt="project-image" width={50} height={50} className={`w-16 h-16 p-1 border-2 cursor-pointer rounded-lg ${selectedImage === image ? "border-primaryDark" : "border-gray-300"}`} />
            </div>
          ))}
        </div>
      </div>
