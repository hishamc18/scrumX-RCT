                        ...
                        if (resetMode) {
                            try {
                                ...
                                const response = await dispatch(
                                    resetPassword({ token, email: enteredEmail, password: values.password })
                                ).unwrap();
                                ...
                                setFieldError("password", error.message || "Password reset failed");
                            }
                        } else {
                            if (!showPasswordField && !showOtpField) {
                                ...
                                try {
                                    ...
                                    setFieldError("password", error.message || "Invalid credentials");
                                }
                            }
                        }
                        setSubmitting(false);
                ...
                >
                    {({ isSubmitting }) => (
                        <Form className="flex flex-col py-2">
                            {resetMode ? (
                                <>
                                    <LoginInputField name="password" placeholder="Enter new password" type="password" />
                                    <LoginInputField
                                        name="confirmPassword"
                                        placeholder="Confirm new password"
                                        type="password"
                                    />
                                    {passwordResetError && <p className="text-red-500 text-xs">{resetPasswordError}</p>}
                                ...
                                <>
                                    ...
                                    {emailExists && showPasswordField && !forgotPasswordSent && (
                                        <div className="relative">
                                            <LoginInputField name="password" placeholder="Enter password" type="password" />
